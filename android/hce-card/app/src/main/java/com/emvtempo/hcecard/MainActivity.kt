...
/**
 * Main activity for the EMV-Tempo HCE card app.
 *
 * Displays card information (PAN, public key) and provides a one-time
 * registration button to register the card on the Tempo testnet.
 */
class MainActivity : AppCompatActivity() {
    ...
    private fun displayCardInfo() {
        // Format PAN for display: XXXX XXXX XXXX XXXX
        ...
        if (P256KeyManager.hasKeyPair()) {
            ...
            Log.i("EMV_CARD_KEY", "PubKeyX=0x$pubKeyX")
            Log.i("EMV_CARD_KEY", "PubKeyY=0x$pubKeyY")
        } else {
    ...
    private fun checkNfc() {
        ...
        when {
            nfcAdapter == null -> {
                setStatus("NFC not available on this device")
            }
            !nfcAdapter.isEnabled -> {
                setStatus("NFC is disabled. Please enable NFC in Settings.")
            }
            else -> {
                setStatus("Ready. Tap phone on terminal to pay.")
            }
        }
    }
    ...
    private fun onRegisterClicked() {
        ...
        // Validate inputs
        if (rpcUrl.isEmpty()) {
            setStatus("Error: RPC URL is required")
            return
        }
        if (contractAddress.isEmpty()) {
            setStatus("Error: Contract address is required")
            return
        }
        if (privateKey.isEmpty()) {
            setStatus("Error: Wallet private key is required")
            return
        }
        if (cardholderAddress.isEmpty()) {
            setStatus("Error: Cardholder address is required")
            return
        }
        if (!P256KeyManager.hasKeyPair()) {
            setStatus("Error: No key pair generated")
            return
        }
